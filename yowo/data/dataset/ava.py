...
class AVA_Dataset(Dataset):
    def __init__(
        ...
        self.frame_list = os.path.join(data_root, frame_list)
        self.annotation_dir = os.path.join(data_root, annotation_dir)
        self.labelmap_file = os.path.join(data_root, annotation_dir, labelmap_file)
        if is_train:
            if gt_box_list is None:
                gt_box_list = "train_gt_box_list"
            if exclusion_file is None:
                exclusion_file = "train_exclusion_file"
            self.gt_box_list = os.path.join(self.annotation_dir, gt_box_list)
            self.exclusion_file = os.path.join(self.annotation_dir, exclusion_file)
        else:
            if gt_box_list is None:
                gt_box_list = "val_gt_box_list"
            if exclusion_file is None:
                exclusion_file = "val_exclusion_file"
            self.gt_box_list = os.path.join(self.annotation_dir, gt_box_list)
            self.exclusion_file = os.path.join(self.annotation_dir, exclusion_file)
