...
class PaFPNELAN(nn.Module):
    def __init__(
        self, 
        in_dims: List[int] = [512, 1024, 1024],
        out_dim: int = 256,
        fpn_size: FPN_SIZE = 'large',
        depthwise: bool = False,
        norm_type: NORM = 'BN',
        ...
        if fpn_size == 'large' or fpn_size == 'huge':
            self.mp1 = DownSample(
                int(128 * width), act_type=act_type,
        ...
        if fpn_size == 'large' or fpn_size == 'huge':
            self.mp2 = DownSample(
                int(256 * width), act_type=act_type,
                norm_type=norm_type, depthwise=depthwise
            )
        elif fpn_size == 'tiny':
        ...
        self.head_conv_1 = Conv(
            int(128 * width), int(256 * width), k=3, p=1,
            act_type=act_type, norm_type=norm_type, depthwise=depthwise
        )
        self.head_conv_2 = Conv(
            int(256 * width), int(512 * width), k=3, p=1,
            act_type=act_type, norm_type=norm_type, depthwise=depthwise
        )
        self.head_conv_3 = Conv(
            int(512 * width), int(1024 * width), k=3, p=1,
            act_type=act_type, norm_type=norm_type, depthwise=depthwise
        )
        # output proj layers
