...
def get_conv2d(c1, c2, k, p, s, d, g, padding_mode: PADDING_MODE = 'ZERO', bias=False):
...
def get_norm(dim: int, norm_type: Optional[NORM] = None):
...
class Conv(nn.Module):
    def __init__(
        ...
        s=1,                                            # padding
        d=1,                                            # dilation
        act_type: Optional[ACTIVATION] = None,          # activation
        norm_type: Optional[NORM] = None,               # normalization
        padding_mode: PADDING_MODE = 'ZERO',            # padding mode: "ZERO" or "SAME"
        depthwise=False
        ):
...
def get_norm3d(dim, norm_type: Optional[NORM_3D]):
    validate_literal_types(norm_type, NORM_3D)
    if norm_type == 'BN':
...
class Conv3d(nn.Module):
    def __init__(
        ...
        g=1,
        act_type: Optional[ACTIVATION] = None,          # activation
        norm_type: Optional[NORM_3D ] = None,           # normalization
        depthwise=False):
